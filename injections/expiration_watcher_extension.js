ExpirationWatcherExtension.prototype = new ShacknewsExtension;
ExpirationWatcherExtension.prototype.constructor = ExpirationWatcherExtension;

function ExpirationWatcherExtension() {
    console.log('Initializing Expiration Watcher.');
    ShacknewsExtension.call(this, "ExpirationWatcher");

    this.extendShacknews();
}

ExpirationWatcherExtension.prototype.extended = function(eventMessage) {
    if($(document).hasClass('expiration-watcher-bar')) {
        //Already installed, nothing to do here.
        return;
    }

    console.log('Expiration watcher extension enabled.');

    ExpirationWatcher.install();
    ShacknewsExtension.getRootPosts().each(function(i, post) {
        ExpirationWatcher.showExpiration(post, post.id.substr(5), true);
    });

};

//Pulled straight (mostly) from ChromeShack. Minus doom marker.
ExpirationWatcher =
{
    // 1000ms * 60s * 60m * 18hr
    ...
    {
        var style = 'Bar'; //getSetting("expiration_watcher_style");

        if (!is_root_post) return;

        //Where are we in the tree?  Is find the right thing to do here? Should it be children?
        var postdate = $(item).find('div.postdate').get(0);
...
var xw = new ExpirationWatcherExtension();